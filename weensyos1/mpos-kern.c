...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		for(i=1; i<NPROCS;i++){
			if(proc_array[i].p_waitid == current->p_pid && proc_array[i].p_state == P_BLOCKED)
	...
	case INT_SYS_WAIT: {
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
			...
			// Change made here for Exercise 4
			proc_array[p].p_state = P_EMPTY; 
			...
			// syscalls rely on storing values in %eax 
			current->p_waitid = p; 
		}
		schedule();
	}
...
do_fork(process_t *parent)
{
	process_t* avail_pd; 
	pid_t offset = 1; 	
	// Find the first empty process descriptor. If none, return -1
	while(1){
		if(offset == NPROCS) 
			// no empty descriptor, e.g. not enough resources
			return -1; 
		avail_pd = &(proc_array[offset]); 
		if(avail_pd->p_state == P_EMPTY)
			break;
		offset++; 
	}
	
	// Initialize process descriptor as a running process
	// Copy parent process's registers & stack 

	// Copy regs
	avail_pd->p_registers = parent->p_registers; 
	// Copy stack
	copy_stack(avail_pd, parent); 
	
	// Housekeeping 
	// Make sure the child has return value of 0
	// Change the state of the child process to runnable 
	// set the child's pid
	// return the child's pid to parent
	avail_pd->p_state = P_RUNNABLE; 
	avail_pd->p_registers.reg_eax = 0;
	avail_pd->p_pid = offset;
	return offset; 
}
...
copy_stack(process_t *dest, process_t *src)
{
	...
	uint32_t byte_offset = src_stack_top - src_stack_bottom; 
	dest_stack_top = PROC1_STACK_ADDR + (dest->p_pid * PROC_STACK_SIZE); 
	dest_stack_bottom = dest_stack_top-(byte_offset);
	// the offset between stack pointers should be the same at creation 
	// time
	
	memcpy(dest, src, byte_offset); 
	// Make the new process's esp point to the bottom of its stack
