...
unsigned long hash(const char *str);	// hashes a string using djb2
static char *rand_string(char *str);	// takes a character array and returns a random string up to MAX_KEY_LENGTH 
long long addspec (struct timespec *ts);	// adds the seconds & nanoseconds time fields of a timespec struct, returns a long long value representing nanoseconds 
...
void * unsynchronizedListFxn (void * args){
	...
	if(clock_gettime(CLOCK_MONOTONIC, &temp)){
		fprintf(stderr, "clock_gettime failed.");
		n_errors++;
	}
	
	if (addspec(&temp) < smallest_start_time)
		smallest_start_time = addspec(&temp);
...
int main(int argc, char *argv[]){
	...
	// Artifact correction -- subtract out thread waiting time 
	if (correct_flag)
